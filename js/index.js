...
async function renderProducts(searchQuery = "", sortOrder = "") {
  ...
  if (!productList) {
    console.error(
      'Product list element not found in the DOM. Ensure the element with id "product-list" exists in your HTML.'
    );
    return;
  }
              ...
              <div class="col-lg-3 col-md-4 col-sm-6 col-6">
                  <div class="ltn__product-item text-center">
                      <div class="product-img">
                          <a href="#" title="Quick View" data-bs-toggle="modal" data-bs-target="#quick_view_modal" onclick="populateQuickViewModal(this,event)" data-product='${JSON.stringify(
                            product
                          ).replace(/'/g, "&apos;")}'><img src="${
      product.thumbnail || "img/product/1.png"
    }" alt="#"></a>
                          
                          <div class="product-hover-action product-hover-action-2">
                              <ul>
                                  <li class="ATC-parent-div" id='atc-btn-${
                                    product._id
                                  }'>
                                      ${
                                        globalCartItems?.some(
                                      ...
                                      `
                                          : `
                                      <a href="#" title="Add to Cart" data-product='${JSON.stringify(
                                        product
                                      ).replace(
                                        /'/g,
                                        "&apos;"
                                      )}' onclick="handleAddToCartClick(this,event,1)">
                                          <div class="d-add-to-cart-div">
                                              <span class="d-block"><i class="fas fa-shopping-cart"></i></span>
                      ...
                      <div class="product-info">
                          <h2 class="product-title"><a href="#" title="Quick View" data-bs-toggle="modal" data-bs-target="#quick_view_modal" onclick="populateQuickViewModal(this,event)" data-product='${JSON.stringify(
                            product
                          ).replace(/'/g, "&apos;")}'>${product.name}</a></h2>
                         <button class="product-title"><a href="#" title="Quick View" data-bs-toggle="modal" data-bs-target="#quick_view_modal" onclick="populateQuickViewModal(this,event)" data-product='${JSON.stringify(
                           product
                         ).replace(/'/g, "&apos;")}'>View Details</a></button>
...
async function handleAddToCartClick(productTag, event, quantity = 1) {
  ...
  if (modal) {
    modal.querySelector(".modal-product-img img").src =
      product?.thumbnail || "img/product/1.png";
    ...
    modal.querySelector(
      ".modal-product-info .added-cart"
    ).innerHTML = `<i class="fas fa-check-circle" style="color: #28a745; font-size: 20px;"></i> Successfully added to your Cart`;
...
function handleAddToCartClick2(productTag, event) {
  ...
  const quickViewModalInstance = bootstrap.Modal.getInstance(
